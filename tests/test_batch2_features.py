...
class RemoteRepoMethodTests(unittest.TestCase):
    ...
    def test_organizer_user_record_lists_only_include_owned_activities(self) -> None:
        from app.api_server import list_checkins, list_registrations, list_schedules

        organizer = User(id="org1", username="organizer", role=Role.ORGANIZER)
        records = [
            {"id": "owned", "user_id": "student1", "activity_id": "act_owned"},
            {"id": "other", "user_id": "student1", "activity_id": "act_other"},
        ]

        def get_activity(activity_id: str) -> dict | None:
            return {
                "act_owned": {"id": "act_owned", "owner_id": organizer.id, "status": ActivityStatus.OPEN.value},
                "act_other": {"id": "act_other", "owner_id": "org2", "status": ActivityStatus.OPEN.value},
            }.get(activity_id)

        with patch("app.api_server.activity_repo") as fake_activity_repo, \
                patch("app.api_server.reg_repo") as fake_reg_repo, \
                patch("app.api_server.schedule_repo") as fake_schedule_repo, \
                patch("app.api_server.checkin_repo") as fake_checkin_repo:
            fake_activity_repo.get.side_effect = get_activity
            fake_reg_repo.list_by_user.return_value = records
            fake_schedule_repo.list_by_user.return_value = records
            fake_checkin_repo.list_by_user.return_value = records

            self.assertEqual(
                list_registrations(user_id="student1", current_user=organizer),
                [records[0]],
            )
            self.assertEqual(
                list_schedules(user_id="student1", current_user=organizer),
                [records[0]],
            )
            self.assertEqual(
                list_checkins(user_id="student1", current_user=organizer),
                [records[0]],
            )
