...
class SchedulingTests(unittest.TestCase):
    def test_schedule_respects_priority_and_capacity(self) -> None:
        ...
        assigned = {(result.user_id, result.slot_id) for _, result in results}
    ...
    def test_first_come_ignores_priority(self) -> None:
        ...
        self.assertEqual(results[0][1].user_id, "u1")
    ...
    def test_lottery_assigns_with_capacity(self) -> None:
        ...
        self.assertEqual(len(results), 2)
        self.assertEqual(len({result.user_id for _, result in results}), 2)
