...
from app.domain.models import ActivityStatus, SignupMode, User
...
class RegistrationPanel(QWidget):
    def __init__(
        ...
        right_col.addWidget(table_group)
        right_col.addWidget(my_reg_group)
    ...
    def _load_my_registrations(self) -> None:
        ...
        for row_index, reg in enumerate(regs):
            self._my_reg_table.setItem(row_index, 0, QTableWidgetItem(reg["id"]))
            activity_name = activities.get(reg["activity_id"], "未知活动")
            self._my_reg_table.setItem(row_index, 1, QTableWidgetItem(activity_name))
            self._my_reg_table.setItem(row_index, 2, QTableWidgetItem(f"志愿 {reg['priority']}"))
